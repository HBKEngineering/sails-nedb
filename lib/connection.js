...
module.exports = base.createChild().addInstanceMethods({
  ...
  getDB: function(collectionID){
      var altCollectionID = String(collectionID).toLowerCase();
      return this.dbs[collectionID] || this.dbs[altCollectionID];
  },
  ...
  loadDB: function (collectionID, cb) {
    ...
    if (!self.config.inMemoryOnly){
        datastoreOptions.filename = path.join(self.dbRoot, collectionID + '.nedb');
    }

    if (!self.dbs[collectionID]) {
      self.dbs[collectionID] = new Datastore(datastoreOptions);
  ...
  select: function (coll, criteria, cb) {
    ...
    var cursor = this.getDB(coll).find(where);
  ...
  update: function (coll, criteria, values, cb) {
    ...
    async.waterfall([
      ...
      function (next) {
        self.getDB(coll).find(query.criteria.where, next);
      },
      ...
      function (records, next) {
        ...
        // Update the records
        self.getDB(coll).update(query.criteria.where, {
          '$set': values
        }, {
          multi: true
        }, next);
  ...
  destroy: function (coll, criteria, cb) {
    ...
    self.getDB(coll).find(query.criteria.where, function (err, records) {
      if (err) return cb(err);

      deletedRecords = records;
    });

    self.getDB(coll).remove(query.criteria.where, {
      multi: true
    }, function (err, numRemoved) {
      if (err) return cb(err);
  ...
  count: function (coll, criteria, cb) {
    ...
    self.getDB(coll).count(query.criteria.where, cb);
  }
});
